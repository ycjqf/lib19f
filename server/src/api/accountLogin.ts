...
router.post("/", async (req, res) => {
  ...
  if (req.session.data) {
    ...
    return req.session.regenerate(() => {
      console.log(`重新登陆成功`);
      return sendJSONStatus<ApiLoginResponse>(res, { code: "OK", message: "登录成功" }, 200);
    });
  }
  ...
  if (loginBody.capacity !== "user")
    return sendJSONStatus<ApiLoginResponse>(
      res,
      { code: "TODO", message: "现在还不允许通过接口注册普通用户外的身份" },
      501
    );
  ...
  if (usingCredential === undefined)
    return sendJSONStatus<ApiLoginResponse>(
      res,
      { code: "PATTERN_UNMATCH", message: "用户名和账号必须有一个存在" },
      403
    );
  ...
  try {
    ...
    // @ts-ignore
    req.session.data = { id: user.id, capacity: "user" };
