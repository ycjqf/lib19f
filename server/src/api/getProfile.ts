...
router.post("/", async (req, res) => {
  ...
  if (typeof params.name === "string" && params.name.trim() !== "") {
    ...
    if (!user) return sendJSONStatus<ApiGetProfileResponse>(res, { code: "NO_SUCH_USER", message: "没有此名称的用户" });
  ...
  if (typeof params.id === "string" && POSITIVE_INTEGER_REGEX.test(params.id)) {
    ...
    if (!user) return sendJSONStatus<ApiGetProfileResponse>(res, { code: "NO_SUCH_USER", message: "没有此ID的用户" });
    const { id, name, avatar } = user;
    return sendJSONStatus<ApiGetProfileResponse>(res, {
      code: "OK",
      message: "用户查询成功",
  ...
  if (typeof params.id !== "number" || params.id <= 0 || params.id !== parseInt(`${params.id}`, 10))
