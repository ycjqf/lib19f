...
router.post("/", async (req, res) => {
  ...
  if (typeof payload === "string" || !payload.id) return sendJSONStatus<ApiAccountReauthResponse>(res, { code: "INVALID_TOKEN", message: "令牌负载无效" });
  const { id, capacity } = payload;
  const accessToken = jwt.sign({ id, capacity }, ACESS_TOKEN_SECRET, { expiresIn: EXPIRE_TIME });
  return sendJSONStatus<ApiAccountReauthResponse>(res, { code: "OK", message: "成功", accessToken: accessToken });
});
